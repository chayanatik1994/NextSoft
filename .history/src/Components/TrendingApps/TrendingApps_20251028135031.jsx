...
const TrendingApps = () => {
  ...
  const toggleVisible = () =>
    setVisible(prev => prev + 4 >= apps.length ? 8 : prev + 4);

  const formatDownloads = num =>
    num >= 1e9 ? `${(num / 1e9).toFixed(1)}B` :
    num >= 1e6 ? `${(num / 1e6).toFixed(0)}M` : num;

  if (loading) return <section className="py-12 text-center">Loading Apps...</section>;

  return (
    <section className="py-12 px-6 text-center">
      <h1 className="text-3xl font-bold mb-2">Trending Apps</h1>
      <p className="text-gray-600 mb-8">Explore all trending apps developed by us</p>
