...
const MyInstallations = () => {
  ...
  const [sortBy, setSortBy] = useState("size"); // State for sorting
  ...
  useEffect(() => {
    const fetchInstalledApps = async () => {
      try {
        ...
        // 3. appsData.json ke pure list se sirf woh apps filter karein
        const allApps = data.apps || []; // Assume JSON has an 'apps' array
  ...
  const handleSort = (sortOption) => {
    ...
    let sortedApps;
    if (sortOption === "size") {
      sortedApps = [...installedApps].sort((a, b) => a.size - b.size);
    } else {
      sortedApps = [...installedApps].sort((a, b) => a.downloads - b.downloads);
    }
    ...
    <div className="p-6 max-w-4xl mx-auto font-sans">
      ...
      <div className="flex justify-between items-center pb-2 border-b border-gray-200">
        ...
        <div className="flex items-center space-x-2 text-sm text-gray-600">
          ...
          <select
            id="sort"
            className="border-none focus:outline-none bg-transparent font-semibold cursor-pointer"
